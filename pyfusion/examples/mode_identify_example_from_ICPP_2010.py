...
m = ['^','o','s']
neq=[neq0,neq1,neq2]
for (p,ind) in enumerate(neq):
    #pl.subplot(3,1,p+1)
    sp(dd,'beta','amp',ind=ind, size_scale=10000,hold=p>0, col='a12',marker=m[p])
    title('n={0}'.format(p))
"""
...
pl.scatter(ds['t_mid'][sind], freq_scale*ds['freq'][sind],dot_size*np.log(ds['amp'][sind]/amp_scale),dd['n_e'][sind],hold=0)
